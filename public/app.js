...
function normaliseFinding(f = {}) {
  return {
    ...
    cvss: sanitiseText(f.cvss || '', 8),
    cwe: sanitiseText(f.cwe || '', 24),
    owasp: sanitiseText(f.owasp || '', 40),
    affectedEndpoint: sanitiseText(f.affectedEndpoint || '', 180),
    ttp: sanitiseText(f.ttp || '', 80),
    description: sanitiseText(f.description || '', 1500),
    recommendation: sanitiseText(f.recommendation || '', 1500),
    discoveredAt: sanitiseDate(f.discoveredAt || ''),
    dueDate: sanitiseDate(f.dueDate || '')
  };
}
...
function renderProjectSubfolder() {
  ...
  projectSubfolder.innerHTML = `<button type="button" class="menu-sub-item">/${escapeHtml(label)}</button>`;
  projectSubfolder.classList.remove('hidden');
  const btn = projectSubfolder.querySelector('button');
